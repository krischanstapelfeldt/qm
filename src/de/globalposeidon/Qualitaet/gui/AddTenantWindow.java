...
/**
 * AddTenantWindow shows the user a window to either add a tenant to the
 * currently selected apartment with "OK" or to cancel the request with "Cancel"
 * 
 * @author Timm Suhl
 *
 */
public class AddTenantWindow extends JDialog {
	...
	public AddTenantWindow(final Apartment apartment) {
		...
		final JLabel descriptionLbl = new JLabel(
				"This will add a new tenant to the apartment with ID: "
						+ apartment.getID());
	...
	private void addTenant(final Apartment apartment) {
		apartment.addTenant(new Tenant(tfFirstname.getText(), tfSurname
				.getText(), tfTel.getText(), tfEmail.getText()));
		dispose();
	}
}
